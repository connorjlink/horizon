...
main_loop_control:
        lw   t2, 24(fp)            # 01842383 # lw      $2,24($fp)
        slti t2,    t2, 4          # 0043a393 # slti    $2,$2, 4
...
telling:
	...
	lw   t2,  0(t2)            # lw      $v0, 0($v0)
        ...
        add  t2,    t3, t2         # 007e03b3 # addu    $2,$3,$2
        lw   t3, 48(fp)            # 03042e03 # lw      $3,48($fp)
        sw   t3,  0(t2)            # 01c3a023 # sw      $3,0($2)
        mv   sp,    fp             # 00800133 # move    $sp,$fp
...
topsort:
        ...
        j    mark_visited
verse:
        addi t2,    fp, 28         # 01c40393 # addiu   $2,$fp,28
        lw   t5, 48(fp)            # 03042f03 # lw      $5,48($fp)
        mv   t4,    t2             # 00700eb3 # move    $4,$2
...
snail:
        ...
        lw   t3,  0(t2)            # lw      $3,0($2)
        lw   t2, 32(fp)            # 02042383 # lw      $2,32($fp)
        lw   t2,  4(t2)            # 0043a383 # lw      $2,4($2)
        mv   t5,    t2             # move    $5,$2
        mv   t4,    t3             # move    $4,$3
...
pat:
        ...
        lw   t2,  8(fp)              # 00842383 # lw      $2,8($fp)
        or   t3,    t3, t2           # 007e6e33 # or      $3,$3,$2
        lw   t2, 16(fp)              # 01042383 # lw      $2,16($fp)
        sw   t3,  0(t2)              # 01c3a023 # sw      $3,0($2)
        mv   sp,    fp               # 00800133 # move    $sp,$fp
        lw   fp, 28(sp)              # 01c12403 # lw      $fp,28($sp)
        addi sp,    sp, 32           # addiu   $sp,$sp,32
        jr   ra                      # jr      $ra
...
justify:
        ...
        lw   t2, 12(fp)              # lw      $2,12($fp)
        addi t2,    t2, 1            # addiu   $2,$2,1
        sw   t2, 12(fp)              # 00742623 # sw      $2,12($fp)
evasive:
        lw   t3, 12(fp)              # 00c42e03 # lw      $3,12($fp)
        lw   t2, 32(fp)              # 02042383 # lw      $2,32($fp)
        slt  t2,    t3, t2           # 007e23b3 # slt     $2,$3,$2
...
representative:
        ...
        lw   t2,  0(t2)              # 0003a383 # lw      $2,0($2)
        sw   t2, 16(fp)              # 00742823 # sw      $2,16($fp)
        lw   t3, 16(fp)              # 01042e03 # lw      $3,16($fp)
        lw   t2,  8(fp)              # 00842383 # lw      $2,8($fp)
        and  t2,    t3, t2           # and     $2,$3,$2
        slt  t2,    x0, t2           # slt     $2,$0,$2
        andi t2,    t2, 0xff         # andi    $2,$2,0x00ff
